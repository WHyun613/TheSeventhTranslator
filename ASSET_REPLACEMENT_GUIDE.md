...
- 通用表现脚本：`res://scripts/ui/interaction_effects.gd`。
- 点击、弹起、飞行、背包轻弹和场景淡入淡出时长集中在脚本顶部常量中，可以统一调整。出口转场固定为：镜头朝出口移动 → 0.35 秒淡黑 → 切换场景 → 0.45 秒淡入 → 恢复控制。
- 拾取音效沿用 AssetCatalog 的 `sfx_item_pickup` 接口。将 WAV 或 OGG 放进工程，然后拖到 `res://content/catalogs/asset_catalog.tres` 的该条目即可。
- 场景物品不需要单独编写动画；只要使用 `SceneHotspot` 并配置物品图片，就会自动获得按下、闪白和弹起反馈。
- 通过场景热点拾取的物品会自动复制当前图像，沿弧线飞向顶部物品栏；原始物品仍由剧情状态控制隐藏，动画不会延迟存档。
- 新增场景出口时，继续使用 `SceneHotspot`，并在 Inspector 的 `Destination Location Id` 填写目标地点 ID。只要该字段非空，就会自动使用统一出口转场；不要在天数脚本里另写一套淡入淡出动画。
