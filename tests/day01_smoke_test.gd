...
func _run() -> void:
	...
	_check(main._dictionary_timeline.visible, "官方词典没有打开独立时间轴 UI。")
	_check(main._dictionary_timeline._page_asset_ids.size() == 3, "官方词典没有固定配置三张时间页。")
	_check(main._dictionary_timeline._unlocked_count == 1, "新游戏的官方词典没有锁定在第一时间页。")
	main._dictionary_timeline._close()
	main.unlock_dictionary_stage(2)
	main._open_dictionary()
	_check(main._dictionary_timeline._unlocked_count == 2, "任务接口没有解锁词典第二时间刻度。")
